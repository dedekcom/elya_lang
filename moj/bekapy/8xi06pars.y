...
CLASSNAME:	IDENTIFIER	{ ddput2($<s>1, "\n"); strcpy(sclassname, $<s>1); ilambdas=-1; }
;
...
/*CALLAMBDA:	CALLOBJECTSTART CALLAMBDASTART METHODBODY1 '}'		{	endkod();	putbuild();	}
	|	CALLOBJECTSTART ARGSMETH CALLAMBDASTART2 METHODBODY1 '}'	{	endkod();	putbuild(); }
;*/
...
void addlocal(char *s)	{
	strcpy(local[idlocal[idkod+1]][idkod+1],s);
	idlocal[idkod+1]++;	
}

...
MEXPR:	EXPR ',' { matchlist(); } MEXPR
	|	EXPR { matchlist();  ddput3("\tjmp ",getloop(-1),"\n"); }
;
MATCHBODY3: EXPR OPSET	{		ddput3("\tcmpst\n\tjne ",pushloop(),"\n\tspop $a\n"); }
 	|	EXPR ',' { pushloop(); pushloop(); matchlist(); } MEXPR OPSET { ddput2(poploop(),":\n\tspop $a\n"); } 
;
...
/*  	|	'?' { pushloop(); }  EXPR { ifpart1(); } OPSET METHODBODY2 */
;
...
EXPR1:	VALUE
	...
	|	STARTCALLEXPR IDENTIFIER ARGSMETH	{	ddput3("\tspop $h\n\tcall $h,",$<s>2,"\n");	}
	|	STARTCALLEXPR IDENTIFIER {	ddput3("\tspop $h\n\tcall $h,",$<s>2,"\n");	}
;

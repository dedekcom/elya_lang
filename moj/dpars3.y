...
OREXP:	EXPR1 BH_OROR	{	checkor();	}
	|	'(' EXPRANDOR ')'  BH_OROR	{	checkor();		}
;
ANDEXP:	EXPR1 BH_ANDAND	{	checkand();	}
	|	'(' EXPRANDOR ')'  BH_ANDAND	{	checkand();	}
;
STARTOREXP:	EXPR1 BH_OROR	{	pushloop();pushloop();	checkor();		}
	|	'(' EXPRANDOR ')'  BH_OROR	{	pushloop();pushloop();	checkor();		}
;
STARTANDEXP:	EXPR1 BH_ANDAND	{	pushloop();pushloop();	checkand();	}
	|	'(' EXPRANDOR ')'  BH_ANDAND	{	pushloop();pushloop();	checkand();	}
;
EXPR:	EXPR1
	|	EXPRANDOR
	|	'(' EXPRANDOR ')'
...
void endandor()	{
	char *c1=getloop(-1), *c2=lastloop(), *c3=pushloop();
	ddput3("\tjmp ",c3,"\n");		// jmp next
